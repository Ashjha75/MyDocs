...
Value is fixed & never overridden

`ordinal()`

Index of the constant

Should avoid using in logic

`valueOf("STRING")`

Returns Enum constant matching the string
...
**Strongly typed** (compiler enforces correct usage)

Just primitive/String, can pass wrong data
...
Can add methods, fields, behaviors

Cannot attach behavior
...
Risky when using integers/strings
